...
from ..qa_serializer import QuestionSerializer
...
class LastQuestionsView(generics.ListAPIView):
    ...
    def get(self,*args,**kwargs): 
        ...
        response.data["popular_tags"] = Question.objects.get_counted_tags()
        response.data["active"] = "all"
        return response
...
class QuestionListView(generics.ListAPIView):
    ...
    def get(self, *args, **kwargs):
        response = super().get(*args, **kwargs)
        response.data["active"] = "unanswered"
        return  response
    ...
    def _vote_handler(self,model , id):
        ...
        try:
            obj.votes.update_or_create(
                user=self.request.user, defaults={"value": value}, )
            obj.count_votes()
